...
import me.rerere.rikkahub.data.script.ScriptChatBridge
import me.rerere.rikkahub.data.script.ScriptChatLite
import me.rerere.rikkahub.data.script.ScriptMessageLite
...
class RikkaScriptChatBridge(
    ...
    override fun findChat(query: String, match: String?, index: Int): ScriptChatLite? {
        val counts = db { messageNodeDAO.getMessageCounts() }
            .associate { it.conversationId to it.count }
        val entities = db { conversationDAO.getAll().first() }
    ...
    override fun getMessages(chatId: String, order: String?, limit: Int?): List<ScriptMessageLite> {
    ...
    override fun createNew(group: String?): ScriptChatLite? {
        return db {
    ...
    override fun sendMessage(chatId: String, content: String, role: String): String? {
        return db {
            val id = runCatching { Uuid.parse(chatId) }.getOrNull() ?: return@db null
            val conversation = conversationRepository.getConversationById(id) ?: return@db null
            val text = content.trim()
            if (text.isEmpty()) return@db null
            val message = if (role.equals("assistant", ignoreCase = true)) {
    ...
    private fun chatLite(entity: ConversationEntity, messageCount: Int): ScriptChatLite {
        return ScriptChatLite(
            id = entity.id,
            title = entity.title,
