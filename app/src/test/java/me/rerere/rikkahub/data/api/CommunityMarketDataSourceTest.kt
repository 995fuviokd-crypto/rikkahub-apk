...
class CommunityMarketDataSourceTest {

    @Test
    fun `parseGitHubUrl handles root tree and blob links`() {
        val root = CommunityMarketDataSource.parseGitHubUrl("https://github.com/exa-labs/exa-mcp-server")
        ...
        val tree = CommunityMarketDataSource.parseGitHubUrl(
        ...
        val blob = CommunityMarketDataSource.parseGitHubUrl(
        ...
        assertEquals(null, CommunityMarketDataSource.parseGitHubUrl("https://example.com/x/y"))
    }
    ...
    fun `extractTarGz unpacks files and directories`() {
        ...
        try {
            CommunityMarketDataSource.extractTarGz(tar, target)
    ...
    fun `zipDirectory packages directory into zip`() {
        ...
        try {
            ...
            val zip = CommunityMarketDataSource.zipDirectory(dir)
    ...
    fun `locateRoot finds repo prefixed dir`() {
        ...
        try {
            ...
            val root = CommunityMarketDataSource.locateRoot(dir, "operit-pack")
    ...
    fun `type label maps operit types`() {
        assertEquals("技能", CommunityMarketDataSource.typeLabel("skill"))
        assertEquals("MCP", CommunityMarketDataSource.typeLabel("mcp"))
        assertEquals("脚本", CommunityMarketDataSource.typeLabel("script"))
        assertEquals("全部", CommunityMarketDataSource.typeLabel("all"))
    }
    ...
    fun `item author accepts object and string forms`() {
        ...
        val asObject = json.decodeFromString<CommunityListItem>(
        ...
        val asString = json.decodeFromString<CommunityListItem>(
    ...
    fun `item publisher accepts object without crashing`() {
        ...
        val real = json.decodeFromString<CommunityListItem>(
            """{"id":"x","title":"t","source":{"kind":"script","url":"https://example.com/a"},
        ...
        val viaPublisher = json.decodeFromString<CommunityListItem>(
            """{"id":"x","title":"t","source":{"kind":"script","url":"https://example.com/a"},
    ...
    @Test
    fun `resolveCommunityHandle prefers github_repo source over assets`() {
        val json = kotlinx.serialization.json.Json { ignoreUnknownKeys = true; explicitNulls = false }
        val skill = json.decodeFromString<CommunityListItem>(
        ...
        val handle = resolveCommunityHandle(skill)
        assertTrue(handle is CommunitySourceHandle.GitHubDir)
        val dir = handle as CommunitySourceHandle.GitHubDir
    ...
    @Test
    fun `resolveCommunityHandle falls back to release asset for package and script`() {
        val json = kotlinx.serialization.json.Json { ignoreUnknownKeys = true; explicitNulls = false }
        val pkg = json.decodeFromString<CommunityListItem>(
        ...
        val handle = resolveCommunityHandle(pkg)
        assertTrue(handle is CommunitySourceHandle.ReleaseAsset)
        val asset = handle as CommunitySourceHandle.ReleaseAsset
        assertEquals("y.toolpkg", asset.assetName)
        assertTrue(asset.url.contains("y.toolpkg"))
    }

    @Test
    fun `resolveCommunityHandle derives github source from id when source missing`() {
        val json = kotlinx.serialization.json.Json { ignoreUnknownKeys = true; explicitNulls = false }
        val item = json.decodeFromString<CommunityListItem>(
            """{"id":"skill-https-github-com-leilaomi-operit-coding-skills","title":"t","source":null,"assets":[]}"""
        )
        val handle = resolveCommunityHandle(item)
        assertTrue(handle is CommunitySourceHandle.GitHubDir)
        val dir = handle as CommunitySourceHandle.GitHubDir
    ...
    @Test
    fun `resolveCommunityHandle fails when no source and no assets`() {
        val json = kotlinx.serialization.json.Json { ignoreUnknownKeys = true; explicitNulls = false }
        val item = json.decodeFromString<CommunityListItem>("""{"id":"script-x","title":"t","source":null,"assets":[]}""")
        try {
            resolveCommunityHandle(item)
    ...
    @Test
    fun `detectCommunityAssetFormat identifies zip gzip and text`() {
        assertEquals(CommunityAssetFormat.ZIP, detectCommunityAssetFormat(byteArrayOf('P'.code.toByte(), 'K'.code.toByte(), 3, 4)))
        assertEquals(CommunityAssetFormat.GZIP, detectCommunityAssetFormat(byteArrayOf(0x1f.toByte(), 0x8b.toByte(), 8, 0)))
        assertEquals(CommunityAssetFormat.TEXT, detectCommunityAssetFormat("/* METADATA */".toByteArray()))
    }

    @Test
    fun `parseScriptMetadata extracts name and localized display`() {
        val script = """
            /*
            METADATA
            {
        ...
        val meta = parseScriptMetadata(script)
    ...
    @Test
    fun `parseScriptMetadata tolerates non-json body`() {
        assertEquals(emptyMap<String, String>(), parseScriptMetadata("function run() {}".toByteArray()))
    }

    @Test
    fun `parseScriptMetaObject returns json object`() {
        val script = "/* METADATA {\"name\":\"x\",\"display_name\":{\"zh\":\"名\"}} */\nfunction run(){}".toByteArray()
        val meta = parseScriptMetaObject(script)
        assertEquals("名", jsonLocalizedString(meta, "display_name"))
        assertEquals("x", jsonLocalizedString(meta, "name"))
        assertEquals(null, parseScriptMetaObject("function run(){}".toByteArray()))
    }
    ...
    @Test
    fun `communityPluginIdFor produces safe stable id`() {
        val id = communityPluginIdFor("package-com-operit-gentle-guardian-artifact-0-6-0")
        assertTrue(id.startsWith("community-"))
        assertEquals(id, communityPluginIdFor("package-com-operit-gentle-guardian-artifact-0-6-0"))
